---
title: "PIEMA Logistic Classifier with Combined Features"
...
This document describes the process of training the PIEMA logistic regression classifier using combined features (sequence + structure). The script generates a number of plots (feature box plots, ROC and PR curves) and outputs (metrics, coefficients) for preliminary analysis based on feature selection criteria. These plots are generated automatically but are further refined and exhibited in `logistic-classifier-plots.R` and `logistic-classifier-analysis.R`, which facilitate more in-depth analysis of optimal model data curation and feature selection approaches.
...
# input.path <- "./analysis/without-cross-reactives/apbs/run2/without-NLV/"
...
Three feature sets will be used for the logistic regression models: all combined features, including average euclidean distance, top subgraph median kas, average spearman correlation, top cosine similarity, kernel count, CDR3 similarity, and full similarity; and limited features, including average euclidean distance, top subgraph median kas, average spearman correlation, and CDR3 similarity (not including full sequence similarity).


```{r recipes, include=FALSE}

# All feature set
model.form.all.features <- shared.specificity ~ avg.euc.dist + avg.kas + top.sg.median.kas + top.sg.euc.dist + avg.spearman + top.cosine.sim + kernel.count + CDR3.similarity + full.similarity

# Limited feature set, including average euclidean distance
model.form.limited.features <- shared.specificity ~ avg.euc.dist + top.sg.median.kas + avg.spearman + kernel.count + CDR3.similarity
...
model1.recipe.limited.features <- recipe(model.form.limited.features, data = model1.train) %>%
    step_dummy(all_nominal(), -all_outcomes()) %>%
    step_normalize(all_predictors()) %>%
    step_downsample(target.var)
model2.recipe.limited.features <- recipe(model.form.limited.features, data = model2.train) %>%
    step_dummy(all_nominal(), -all_outcomes()) %>%
    step_normalize(all_predictors()) %>%
    step_downsample(target.var)
...
model1.workflow.limited.features <- workflow() %>%
    add_recipe(model1.recipe.limited.features) %>%
    add_model(logreg.model)
model2.workflow.limited.features <- workflow() %>%
    add_recipe(model2.recipe.limited.features) %>%
    add_model(logreg.model)
...
model1.fit.limited.features <- model1.workflow.limited.features %>%
    fit_resamples(
        resamples = model1.cv.folds,
        control = control,
        metrics = classification.metrics)

model2.fit.limited.features <- model2.workflow.limited.features %>%
    fit_resamples(
        resamples = model2.cv.folds,
        control = control,
        metrics = classification.metrics)
...
model1.train.pred.limited.features <- model1.fit.limited.features %>%
    collect_predictions()
model2.train.pred.limited.features <- model2.fit.limited.features %>%
    collect_predictions()
...
model1.train.metrics.limited.features <- model1.fit.limited.features %>%
    collect_metrics()
model2.train.metrics.limited.features <- model2.fit.limited.features %>%
    collect_metrics()
...
model1.train.metrics <- model1.train.metrics.all.features %>%
    ...
    bind_cols(model1.train.metrics.limited.features %>% 
        select(mean) %>% 
        mutate(mean = round(mean, 3)) %>%
        dplyr::rename(limited.features = mean)) %>%
...
model2.train.metrics <- model2.train.metrics.all.features %>%
    ...
    mutate(mean = round(mean, 3)) %>%
    dplyr::rename(all.features = mean) %>%
    bind_cols(model2.train.metrics.limited.features %>% 
        select(mean) %>% 
        mutate(mean = round(mean, 3)) %>%
        dplyr::rename(limited.features = mean)) %>%
...
datatable(model1.train.metrics %>% dplyr::rename(metrics = .metric), 
    ...
    caption = paste0("Training metrics: True positive pairs vs. decoy pairs, tuned penalty of ", model1.best.tune.penalty))

datatable(model2.train.metrics, extensions = c('Scroller'),
    options = list(dom='t', scrollX=TRUE, deferRender=TRUE, scrollY=200, scroller=TRUE),
    caption = paste0("Training metrics: True positive pairs vs. unlikely + decoy pairs, tuned penalty of ", model2.best.tune.penalty))
...
model1.test.fit.limited.features <- model1.workflow.limited.features %>%
    last_fit(model1.split, 
        metrics = classification.metrics)
model2.test.fit.limited.features <- model2.workflow.limited.features %>%
    last_fit(model2.split, 
        metrics = classification.metrics)
...
model1.test.pred.limited.features <- model1.test.fit.limited.features %>%
    collect_predictions() %>%
    left_join((model1.data %>% 
        mutate(row.number = row_number())), 
        by = join_by("shared.specificity", ".row" == "row.number"))

model2.test.pred.limited.features <- model2.test.fit.limited.features %>%
    collect_predictions() %>%
    left_join((model2.data %>% 
        mutate(row.number = row_number())), 
        by = join_by("shared.specificity", ".row" == "row.number"))
...
model1.test.metrics.limited.features <- model1.test.pred.limited.features %>%
    classification.metrics(truth = shared.specificity,
        estimate = .pred_class,
        .pred_Yes,
        event_level = 'second')
model2.test.metrics.limited.features <- model2.test.pred.limited.features %>%
    classification.metrics(truth = shared.specificity,
        estimate = .pred_class,
        .pred_Yes,
        event_level = 'second')
...
model1.test.metrics <- model1.test.metrics.all.features %>%
    ...
    bind_cols(model1.test.metrics.limited.features %>% 
        select(.estimate) %>% 
        mutate(.estimate = round(.estimate, 3)) %>%
        dplyr::rename(limited.features = .estimate)) %>%
...
model2.test.metrics <- model2.test.metrics.all.features %>%
    ...
    mutate(.estimate = round(.estimate, 3)) %>%
    dplyr::rename(all.features = .estimate) %>%
    bind_cols(model2.test.metrics.limited.features %>% 
        select(.estimate) %>% 
        mutate(.estimate = round(.estimate, 3)) %>%
        dplyr::rename(limited.features = .estimate)) %>%
...
model1.cm.limited.features <- confusionMatrix(data = model1.test.pred.limited.features$.pred_class, 
    reference = model1.test[[target.var]], 
    positive = positive.class)  
...
model2.cm.limited.features <- confusionMatrix(data = model2.test.pred.limited.features$.pred_class,
    reference = model2.test[[target.var]], 
    positive = positive.class)  
...
bind_rows((model1.test.pred.all.features %>%
    ...
    (model1.test.pred.limited.features %>%
        roc_curve(truth = shared.specificity, .pred_Yes, event_level = "second") %>%
        mutate(model = paste("Limited features\nROC AUC = ", 
            ROC_AUC = round(model1.test.metrics %>% filter(.metric == 'roc_auc') %>% pull(limited.features), 3)))),
...
ggsave(paste0(out.path, "combined_features_model1_roc_curve.png"), width = 10, height = 8)
...
bind_rows((model1.test.pred.all.features %>%
    ...
    (model1.test.pred.limited.features %>%
        pr_curve(truth = shared.specificity, .pred_Yes, event_level = "second") %>%
        mutate(model = paste("Limited features\nPR AUC = ", 
            PR_AUC = round(model1.test.metrics %>% filter(.metric == 'pr_auc') %>% pull(limited.features), 3)))),
    (model1.test.pred.best.tune %>%
        pr_curve(truth = shared.specificity, .pred_Yes, event_level = "second") %>%
        mutate(model = paste("Best tuned model\nPR AUC = ", 
...
ggsave(paste0(out.path, "combined_features_model1_pr_curve.png"), width = 10, height = 6)
...
bind_rows((model2.test.pred.all.features %>%
    ...
    (model2.test.pred.limited.features %>%
        roc_curve(truth = shared.specificity, .pred_Yes, event_level = "second") %>%
        mutate(model = paste("Limited features\nROC AUC = ", 
            ROC_AUC = round(model2.test.metrics %>% filter(.metric == 'roc_auc') %>% pull(limited.features), 3)))),
...
ggsave(paste0(out.path, "combined_features_model2_roc_curve.png"), width = 10, height = 8)
...
bind_rows((model2.test.pred.all.features %>%
    ...
    (model2.test.pred.limited.features %>%
        pr_curve(truth = shared.specificity, .pred_Yes, event_level = "second") %>%
        mutate(model = paste("Limited features\nPR AUC = ", 
            PR_AUC = round(model2.test.metrics %>% filter(.metric == 'pr_auc') %>% pull(limited.features), 3)))),
    (model2.test.pred.best.tune %>%
        pr_curve(truth = shared.specificity, .pred_Yes, event_level = "second") %>%
        mutate(model = paste("Best tuned model\nPR AUC = ", 
...
ggsave(paste0(out.path, "combined_features_model2_pr_curve.png"), width = 10, height = 8)
...
model1.epitope.metrics <- (model1.test.pred.all.features %>%
    ...
    mutate(.estimate = round(.estimate, 3)) %>% 
    dplyr::rename(all.features = .estimate)) %>%
    bind_cols((model1.test.pred.limited.features %>%
        group_by(ref.epitope) %>%
        classification.metrics(truth = shared.specificity,
            estimate = .pred_class,
            .pred_Yes,
...
model2.epitope.metrics <- (model2.test.pred.all.features %>%
    ...
    mutate(.estimate = round(.estimate, 3)) %>% 
    dplyr::rename(all.features = .estimate)) %>%
    bind_cols((model2.test.pred.limited.features %>%
        mutate(epitope = ref.epitope) %>%
        bind_rows(model2.test.pred.limited.features %>%
            filter(ref.epitope != samp.epitope) %>%
            mutate(epitope = samp.epitope)) %>%
        group_by(epitope) %>%
        classification.metrics(truth = shared.specificity,
...
# ROC + PR AUC
# Tuned model is not shown for the sake of clarity, as its curves are nearly identical to that of the all features model
...
ggsave(paste0(out.path, "combined_features_model1_per_epitope_roc_curve.png"), width = 15, height = 12)
...
ggsave(paste0(out.path, "combined_features_model1_per_epitope_pr_curve.png"), width = 15, height = 12)
...
ggsave(paste0(out.path, "combined_features_model2_per_epitope_roc_curve.png"), width = 15, height = 12)
...
ggsave(paste0(out.path, "combined_features_model2_per_epitope_pr_curve.png"), width = 15, height = 12)
...
write.csv(model1.train.pred.all.features, paste0(out.path, "combined_features_model1_train_pred_all_features.csv"), row.names = FALSE)
write.csv(model2.train.pred.all.features, paste0(out.path, "combined_features_model2_train_pred_all_features.csv"), row.names = FALSE)
write.csv(model1.train.pred.limited.features, paste0(out.path, "combined_features_model1_train_pred_limited_features.csv"), row.names = FALSE)
write.csv(model2.train.pred.limited.features, paste0(out.path, "combined_features_model2_train_pred_limited_features.csv"), row.names = FALSE)
write.csv(model1.best.tune.pred.all.features, paste0(out.path, "combined_features_model1_train_pred_best_tune.csv"), row.names = FALSE)
write.csv(model2.best.tune.pred.all.features, paste0(out.path, "combined_features_model2_train_pred_best_tune.csv"), row.names = FALSE)

write.csv(model1.train.metrics, paste0(out.path, "combined_features_model1_train_metrics.csv"), row.names = FALSE)
write.csv(model2.train.metrics, paste0(out.path, "combined_features_model2_train_metrics.csv"), row.names = FALSE)

write.csv(model1.test.pred.all.features, paste0(out.path, "combined_features_model1_test_pred_all_features.csv"), row.names = FALSE)
write.csv(model2.test.pred.all.features, paste0(out.path, "combined_features_model2_test_pred_all_features.csv"), row.names = FALSE)
write.csv(model1.test.pred.limited.features, paste0(out.path, "combined_features_model1_test_pred_limited_features.csv"), row.names = FALSE)
write.csv(model2.test.pred.limited.features, paste0(out.path, "combined_features_model2_test_pred_limited_features.csv"), row.names = FALSE)
write.csv(model1.test.pred.best.tune, paste0(out.path, "combined_features_model1_test_pred_best_tune.csv"), row.names = FALSE)
write.csv(model2.test.pred.best.tune, paste0(out.path, "combined_features_model2_test_pred_best_tune.csv"), row.names = FALSE)

write.csv(model1.test.metrics, paste0(out.path, "combined_features_model1_test_metrics.csv"), row.names = FALSE)
write.csv(model2.test.metrics, paste0(out.path, "combined_features_model2_test_metrics.csv"), row.names = FALSE)

write.csv(model1.epitope.metrics, paste0(out.path, "combined_features_model1_epitope_metrics.csv"), row.names = FALSE)
write.csv(model2.epitope.metrics, paste0(out.path, "combined_features_model2_epitope_metrics.csv"), row.names = FALSE)

write.csv(model1.coefficients, paste0(out.path, "combined_features_model1_coefficients.csv"), row.names = FALSE)
write.csv(model2.coefficients, paste0(out.path, "combined_features_model2_coefficients.csv"), row.names = FALSE)
