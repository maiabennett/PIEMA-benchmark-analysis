...
# All feature set: all features, minus those which are highly correlated (e.g., top subgraph median KAS with top KAS and average KAS)
model.form.all.features <- shared.specificity ~ avg.kas + avg.euc.dist + avg.spearman + top.sg.euc.dist + top.sg.cosine.sim + kernel.count + top.kas + CDR3.similarity + full.similarity
...
# Basic LogReg for all features and PCA
logreg.model <- 
    logistic_reg() %>%
    set_engine("glm") %>%
    set_mode("classification") 

# LASSO LogReg for tuned features
logreg.model.tune <- 
...
datatable(model1.coefficients,
    extensions = c('Scroller'),
    options = list(dom='t', scrollX=TRUE, deferRender=TRUE, scrollY=200, scroller=TRUE),
    caption = "Model 1: Coefficients for all and tuned feature sets")

datatable(model2.coefficients,
    extensions = c('Scroller'),
    options = list(dom='t', scrollX=TRUE, deferRender=TRUE, scrollY=200, scroller=TRUE),
    caption = "Model 2: Coefficients for all and tuned feature sets")
...
write.csv(model1.train.pred.pca.features, paste0(out.path, "combined_features_model1_train_pred_pca_features.csv"), row.names = FALSE)
write.csv(model2.train.pred.pca.features, paste0(out.path, "combined_features_model2_train_pred_pca_features.csv"), row.names = FALSE)
...
write.csv(model1.test.pred.pca.features, paste0(out.path, "combined_features_model1_test_pred_pca_features.csv"), row.names = FALSE)
write.csv(model2.test.pred.pca.features, paste0(out.path, "combined_features_model2_test_pred_pca_features.csv"), row.names = FALSE)
